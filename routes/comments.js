...
router.post('/osr/deleteComment', authenticateToken, (req, res) => {
    ...
    db.execute('Select res_id from comments where comment_id = ?', [comment_id])
        .then(res1 => {
            const rs = res1[0]
            console.log(rs[0].res_id);
            db.execute('UPDATE resources SET comments = comments - 1 WHERE res_id = ?', [rs[0].res_id])
                .then(res2 => {
...
router.post('/osr/addComment', authenticateToken, (req, res) => {
    ...
    db.execute('INSERT INTO COMMENTS(res_id, user_id, comment) VALUES(?, ?, ?)', [res_id, id, comment])
        .then(result => {
            db.execute('UPDATE resources SET comments = comments + 1 WHERE res_id = ?', [res_id])
                .then(res1 => {
                    console.log(res1);
                    res.redirect('/osr/activities');
                })
